...
function authenticator.htmlauth(validator, accs, default)
	...
	local key  = luci.http.formvalue("getcontent") or 0

	require "io"
	require "luci.http"

	local fp = io.open("/tmp/authtxt","r+")
	if not fp then
		fp = io.open("/tmp/authtxt","w+")
		fp:write(0)
	end
	...
	require("luci.template")
	context.path = {}
	luci.template.render("sysauth", {duser=default, fuser=user, refresh=o})
	return false
