...
--	if page.inreq then
		...
		page = entry({"admin", "network", "network_connection_status"}, call("network_connection_status"), nil)
		page.leaf = true

		page = entry({"admin", "network", "internet_connection_status"}, call("internet_connection_status"), nil)
		page.leaf = true

        page = entry({"admin", "network", "get_wireless_devices"}, call("get_wifi_devices"), nil)
        page.leaf = true

        page = entry({"admin", "network", "get_connected_devices"}, call("get_connected_devices"), nil)
        page.leaf = true

        page = entry({"admin", "network", "get_wifi_networks"}, call("get_wifi_networks"), nil)
        page.leaf = true

        page = entry({"admin", "network", "create_wireless_network"}, call("create_wireless_network"), nil)
        page.leaf = true
--	end
end	


function create_wireless_network()
	local ssid = luci.http.formvalue("ssid")
	local encryption = luci.http.formvalue("encryption")
	local key = luci.http.formvalue("key")
	local key_show = luci.http.formvalue("key_show")
	local device = luci.http.formvalue("device")

	require "luci.sys"

	luci.sys.exec("uci add wireless wifi_iface")

	luci.sys.exec("uci set wireless.@wifi_iface[-1].device=" .. device)
	luci.sys.exec("uci set wireless.@wifi_iface[-1].ssid=" .. ssid)
	luci.sys.exec("uci set wireless.@wifi_iface[-1].encryption=" .. encryption)
	luci.sys.exec("uci set wireless.@wifi_iface[-1].key=" .. key)
	luci.sys.exec("uci set wireless.@wifi_iface[-1].disabled=1")

	luci.sys.exec("uci commit")

	luci.http.prepare_content("application/json")
	luci.http.write_json(ssid)
end
...
function get_connected_devices()
	...
	local dhcp4_devices = s.dhcp_leases()
	local dhcp6_devices = s.dhcp6_leases()
	...
	for name, data in pairs(luci.sys.net.deviceinfo()) do
		devicesinfo[#devicesinfo+1] = {name = name,
										rx_bytes = data[1],
										tx_bytes = data[9]
									}
	end
	...
	if dhcp4_devices then
		for i, device in ipairs(dhcp4_devices) do
			dhcp4_devices[i].info = arptable
		end
	end

	if dhcp6_devices then
		for i, device in ipairs(dhcp6_devices) do
			--info = ntm.get_status_by_address(device.ipaddr)
			--dhcp6_devices[i].info = info
		end
	end

	luci.http.prepare_content("application/json")
	luci.http.write('[')
	luci.http.write_json(dhcp4_devices)
	luci.http.write(',')
	luci.http.write_json(dhcp6_devices)
	luci.http.write(']')
end
...
function internet_connection_status()
	-- diag_ping \ diag_traceroute \ diag_ping6 \diag_traceroute6
	local ret = {}
